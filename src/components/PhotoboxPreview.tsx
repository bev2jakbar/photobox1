...
// @ts-ignore
import nutrijellFrameBg from '../assets/images/nj.jpg';
// @ts-ignore
import overlayNutrijellImg from '../assets/images/ov_nj.png';
// @ts-ignore
import angetSariFrameBg from '../assets/images/bc_anget.png';
// @ts-ignore
import overlayAngetSariImg from '../assets/images/ov_anget.png';
...
export default function PhotoboxPreview({
  ...
  const isCustomTemplateFrame = activeFrame.id === 'tts1' || activeFrame.id === 'tts2' || activeFrame.id === 'tts3' || activeFrame.id === 'gummyberry' || activeFrame.id === 'strawberry' || activeFrame.id === 'popice_candy' || activeFrame.id === 'etawa' || activeFrame.id === 'nutrijell' || activeFrame.id === 'anget_sari';
  ...
  const getCustomFrameBg = () => {
    ...
    if (activeFrame.id === 'nutrijell') return nutrijellFrameBg;
    if (activeFrame.id === 'anget_sari') return angetSariFrameBg;
    return null;
  };
  ...
  const renderCardToCanvas = async (setLoadingStatus: (msg: string) => void) => {
    ...
    if (isCustomTemplateFrame) {
      try {
        ...
        if (activeFrame.id === 'tts2') {
          ...
          frameLabel = 'Etawa';
        } else if (activeFrame.id === 'nutrijell') {
          frameImgBg = nutrijellFrameBg;
          frameLabel = 'Nutrijell';
        } else if (activeFrame.id === 'anget_sari') {
          frameImgBg = angetSariFrameBg;
          frameLabel = 'Anget Sari';
        }
    ...
    const isSinglePhoto = currentPhotos.length === 1;
    ...
    for (let i = 0; i < maxPhotosToDraw; i++) {
      ...
      // Load Image
      const img = new Image();
      if (!currentPhotos[i].startsWith('data:')) {
        img.crossOrigin = 'anonymous';
      }
    ...
    if (isCustomTemplateFrame) {
      ...
      if (activeFrame.id === 'nutrijell') {
        try {
          const stickerImg = await loadImage(overlayNutrijellImg);
          const stickerWidth = cardWidth; // 100% of total card width
          const stickerHeight = stickerWidth * (stickerImg.height / stickerImg.width);
          const stickerX = 0; // Left-aligned
          const stickerY = cardHeight - stickerHeight; // Bottom aligned with the very bottom of the card
          ctx.save();
          ctx.drawImage(stickerImg, stickerX, stickerY, stickerWidth, stickerHeight);
          ctx.restore();
        } catch (err) {
          console.error('Gagal memuat stiker Nutrijell kustom:', err);
        }
      }

      if (activeFrame.id === 'anget_sari') {
        try {
          const stickerImg = await loadImage(overlayAngetSariImg);
          const stickerWidth = cardWidth; // 100% of total card width
          const stickerHeight = stickerWidth * (stickerImg.height / stickerImg.width);
          const stickerX = 0; // Left-aligned
          const stickerY = cardHeight - stickerHeight; // Bottom aligned with the very bottom of the card
          ...
          console.error('Gagal memuat stiker Anget Sari kustom:', err);
        }
      }
    }
      ...
      <div className="flex-1 max-w-md mx-auto w-full px-5 py-6 flex flex-col items-center gap-6">
        ...
        <div className="w-full flex justify-center">
          ...
          >
            ...
            {activeFrame.id === 'nutrijell' && (
              <img
                src={overlayNutrijellImg}
                alt="Sticker Nutrijell"
                className="absolute z-40 pointer-events-none select-none"
                style={{
                  left: '0%',
                  width: '100%',
            ...
            {activeFrame.id === 'anget_sari' && (
              <img
                src={overlayAngetSariImg}
                alt="Sticker Anget Sari"
                className="absolute z-40 pointer-events-none select-none"
                style={{
                  left: '0%',
                  width: '100%',
            ...
            >
              {currentPhotos.map((photo, i) => (
                <label
                  key={i}
                  className={`group relative rounded-xl overflow-hidden transition-all duration-300 w-full h-full cursor-pointer flex ${
                    isCustomTemplateFrame ? 'border-0' : 'border-2'
                  }`}
                  style={{
                    backgroundColor: isCustomTemplateFrame ? activeFrame.canvasBg : '#18181b',
                    borderColor: !isCustomTemplateFrame ? activeFrame.canvasBorder : undefined
                  }}
                >
                  <img
                    src={photo}
                  ...
                  />
                  {/* Hover/Tap Overlay to replace the photo */}
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center gap-1 transition-all duration-200 z-10">
                    <ImageIcon size={18} className="text-pink-400" />
                    <span className="text-[9px] font-black uppercase tracking-wider text-white bg-zinc-900/90 px-1.5 py-0.5 rounded-md border border-zinc-700">
                      Ganti Foto {i + 1}
                    </span>
                  </div>
                  {/* Subtle corner edit icon for better discoverability on touch devices */}
                  <div className="absolute bottom-1 right-1 w-5 h-5 rounded-full bg-black/75 backdrop-blur-sm border border-white/10 flex items-center justify-center text-zinc-300 group-hover:text-pink-400 transition-all duration-200 z-10">
                    <ImageIcon size={10} />
                  </div>
